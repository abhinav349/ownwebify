...
export function Hero() {
  ...
  const fallback = img(demoImages.clinic.hero, 1920, 70);

  return (
    <section
      id="top"
      ref={sectionRef}
      className="relative h-[100svh] min-h-[640px] w-full overflow-hidden bg-[#0a1210]"
    >
      {reducedMotion ? (
        <Image
          src={fallback.src}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#0a1210] via-[#0a1210]/10 to-[#0a1210]/45 pointer-events-none" />
      ...
      >
        <Reveal>
          <p className="eyebrow-demo">CarePlus Medical</p>
        </Reveal>
        <Reveal delay={0.08}>
          <h1 className="mt-6 font-display text-5xl md:text-8xl leading-[1.05] text-balance">
            Care that feels
            <br />
            <span style={{ color: ACCENT }}>human again</span>
          </h1>
        </Reveal>
        <Reveal delay={0.16}>
          <p className="mt-8 text-base md:text-lg text-white/60 max-w-lg mx-auto leading-relaxed font-light">
            ...
            with same-day appointments and a team that actually listens.
          </p>
        </Reveal>
        <Reveal delay={0.24}>
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Magnetic>
              <a
                ...
                style={{ backgroundColor: ACCENT, color: "#0a1210" }}
              >
                Book Appointment
              </a>
            </Magnetic>
        ...
        <Reveal delay={0.32}>
          <div className="mt-16 flex items-center justify-center gap-6 sm:gap-12 text-center">
            <div>
              ...
              <p className="text-xs text-white/40 uppercase tracking-wider">25K+ Patients</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div>
              <ShieldCheck className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">Board Certified</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div>
              <Clock className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">Same-Day Visits</p>
            </div>
          </div>
        </Reveal>
      </motion.div>
