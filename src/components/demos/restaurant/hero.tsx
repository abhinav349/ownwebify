...
import { ChevronDown, Wine, Flame, Award } from "lucide-react";
...
export function Hero() {
  ...
  const fallback = img(demoImages.restaurant.hero, 1920, 70);

  return (
    <section
      id="top"
      ref={sectionRef}
      className="relative h-[100svh] min-h-[640px] w-full overflow-hidden bg-[#1a0a0a]"
    >
      {reducedMotion ? (
        <Image
          src={fallback.src}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#1a0a0a] via-[#1a0a0a]/10 to-[#1a0a0a]/50 pointer-events-none" />
      ...
      >
        <Reveal>
          <p className="eyebrow-demo">Michelin Recognized</p>
        </Reveal>
        <Reveal delay={0.08}>
        ...
        <Reveal delay={0.16}>
          <p className="mt-8 text-base md:text-lg text-white/60 max-w-lg mx-auto leading-relaxed font-light">
            ...
            celebrating the finest ingredients and time-honored technique.
          </p>
        </Reveal>
        <Reveal delay={0.24}>
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Magnetic>
              <a
                ...
                style={{ backgroundColor: ACCENT, color: "#1a0a0a" }}
              >
                Reserve a Table
              </a>
            </Magnetic>
        ...
        <Reveal delay={0.32}>
          <div className="mt-16 flex items-center justify-center gap-6 sm:gap-12 text-center">
            <div>
              <Wine className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">200+ Wines</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div>
              <Flame className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">Open Kitchen</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div>
              <Award className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">Award Winning</p>
            </div>
          </div>
        </Reveal>
      </motion.div>
