...
import { ChevronDown, Coffee, Leaf, Clock } from "lucide-react";
...
export function Hero() {
  ...
  const fallback = img(demoImages.cafe.hero, 1920, 70);

  return (
    <section
      id="top"
      ref={sectionRef}
      className="relative h-[100svh] min-h-[640px] w-full overflow-hidden bg-[#120d0a]"
    >
      {reducedMotion ? (
        <Image
          src={fallback.src}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[#120d0a] via-[#120d0a]/10 to-[#120d0a]/40 pointer-events-none" />
      ...
      >
        <Reveal>
          <p className="eyebrow-demo">Artisan Coffee Roasters</p>
        </Reveal>
        <Reveal delay={0.08}>
        ...
        <Reveal delay={0.16}>
          <p className="mt-8 text-base md:text-lg text-white/60 max-w-md mx-auto leading-relaxed font-light">
            Single-origin beans, roasted in-house daily. A quiet ritual for
            those who appreciate the art of a perfect cup.
          </p>
        </Reveal>
        <Reveal delay={0.24}>
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Magnetic>
              <a
                ...
                style={{ backgroundColor: ACCENT, color: "#120d0a" }}
              >
                Explore Menu
              </a>
            </Magnetic>
        ...
        <Reveal delay={0.32}>
          <div className="mt-16 flex items-center justify-center gap-6 sm:gap-12 text-center">
            <div>
              <Coffee className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">12+ Origins</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div>
              <Leaf className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">Ethically Sourced</p>
            </div>
            <div className="h-8 w-px bg-white/10" />
            <div>
              <Clock className="h-5 w-5 mx-auto mb-2" style={{ color: `${ACCENT}99` }} />
              <p className="text-xs text-white/40 uppercase tracking-wider">Roasted Daily</p>
            </div>
          </div>
        </Reveal>
      </motion.div>
